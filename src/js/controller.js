...
import resultsView from "./views/searchView/resultsView.js";
import popularView from "./views/homepageView/popularView.js";
import nowPlayingView from "./views/homepageView/nowPlayingView.js
import topRatedView from "./views/homepageView/topRatedView.js";
import upcomingView from "./views/homepageView/upcomingView.js";
...
export const controlMovie = async function (id) {
  try {
    if (!id) return;
...
const init = function () {
  ...
  movieView.addHandlerRender(() => controlMovie(window.location.hash.slice(1))); // Pass current hash ID to controlMovie
  ...
  movieView.addHandlerRecommendationClick((id) => {
    if (id) {
      window.location.hash = `#${id}`; // Update hash to load the selected movie
    }
  });
